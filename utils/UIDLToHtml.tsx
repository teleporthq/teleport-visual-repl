  ...
  Object.keys(object).forEach(key => {
    if (typeof object[key].content !== "string") {
      ...
      result +=
        camelCaseToDash(key) +
        ":" +
        eliminateQuotes(object[key].content) +
        "; ";
    }
  });
  return result;
...
function getOuterStyleContents(
  className: string,
  object: object,
      ...
      Object.keys(entry.elementInfo).forEach(key => {
        ...
        if (key === "style") {
          if (typeof entry.elementInfo["name"] !== "undefined") {
            styleResult += getOuterStyleContents(
              entry.elementInfo["name"] + counter,
        ...
        if (typeof entry.elementInfo[key] === "string") {
          accumulator +=
            key === "name"
              ? entry.elementInfo[key] + counter + " "
              : entry.elementInfo[key] + " ";
          return;
        }
        accumulator += '"';
        accumulator = getInlineStyleObjectContents(
          accumulator,
          entry.elementInfo[key]
        );
        accumulator += '"';
